 ...
 * @param {(orderedIds: string[]) => Promise<unknown>} props.onReorderEnd
 * @param {(question: object) => void} props.onEdit
...
function QuestionSlot({
  ...
  const title = question.content?.en?.title?.trim() ?? '';
  const body = question.content?.en?.body?.trim() ?? '';
  const preview = title || body || '(empty)';
  const bodySnippet = title && body && title !== body ? body : null;
        ...
        >
          ...
          <div className="intake-step-body">
            <div className="intake-step-meta">
              ...
              <span className="intake-type-pill">{typeLabel}</span>
              {!question.required ? <span className="intake-badge">Optional</span> : null}
              {!question.isActive ? (
            ...
            {bodySnippet ? (
              <p className="intake-step-snippet" title={bodySnippet}>
                {bodySnippet}
              </p>
            ) : null}
          </div>
